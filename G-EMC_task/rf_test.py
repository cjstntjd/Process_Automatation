...
def test():
    document = Document('C:\\Users\\84430\\Desktop\\FlaskApp - 복사본\\tlqkf.docx')
...
def introduction(i,document,test_mode,f,temp):
    ...
    a.text = test_mode
    #'2.4G WLAN (802.11b)' # 조작하고 싶은 주파수 대역대 (선택한 id이름으로 )
    ...
    a.text = f[0] + '  ' + f[1] + '  ' + f[2]
    #2472MHz' # F1 , F2, F3 각각 엑셀이랑 연동되도록
    ...
    hdr_cells[1].text = temp[0] #'+25℃'
    hdr_cells[2].text = temp[1] #'+50℃'
    hdr_cells[3].text = temp[2] #'-10℃'
    hdr_cells[4].text = temp[3] #'+35℃, 95%'
...
def single_table(d,ws1_name,data,regulation):
    #r : 14 c: 8
    ...
    ws1_name_spc = ""
    for idz,z in enumerate(ws1_name):
        ws1_name_spc += z + '  '

    a.text = ws1_name_spc
    #'주   파   수   허   용   편   차' # 선택한 것들 (test_mode)
    a.width = Cm(1.41)
    a.paragraphs[0].alignment = WD_ALIGN_PARAGRAPH.CENTER #가운데 정렬
    a.vertical_alignment = WD_ALIGN_VERTICAL.CENTER #가운데 정렬
    a.paragraphs[0].runs[0].bold = True
...
##################### F3
    ...
    for r,row in enumerate(data):
        for i in range(3): # size 3
            table.cell(r+2,i+3).text = str(row[i])
            # 열 3 4 5
            # 행 2 <14
            tp = table.cell(r+2,i+3)
            tp.paragraphs[0].alignment = WD_ALIGN_PARAGRAPH.CENTER #가운데 정렬
            tp.vertical_alignment = WD_ALIGN_VERTICAL.CENTER #가운데 정렬

    # limit
    for rw in range(2,14):
        table.rows[rw].height = Pt(27)

    table.cell(2,6).text = regulation
    table.cell(2,6).paragraphs[0].alignment = WD_ALIGN_PARAGRAPH.CENTER #가운데 정렬
    table.cell(2,6).vertical_alignment = WD_ALIGN_VERTICAL.CENTER #가운데 정렬

def double_table(d,ws1_name, data1, regulation1, ws2_name, data2, regulation2):
    #doc, ws1_name, data1, regulation1, ws2_name, data2, regulation2
    #r : 14 c: 8
...
############################################################################# no introduction
    ...
    ws2_name_spc = ""
    for idz,z in enumerate(ws2_name):
        ws2_name_spc += z + '  '

    a.text = ws2_name_spc
    #'점  유  주  파  수  대  역  폭' # 선택한 것들
    a.width = Cm(1.41)
    a.paragraphs[0].alignment = WD_ALIGN_PARAGRAPH.CENTER #가운데 정렬
    a.vertical_alignment = WD_ALIGN_VERTICAL.CENTER #가운데 정렬
    a.paragraphs[0].runs[0].bold = True
...
##################### F3
    ...
    for r,row in enumerate(data2):
        for i in range(3): # size 3
            table.cell(r+14,i+3).text = str(row[i])
            tp = table.cell(r+14,i+3)
            tp.paragraphs[0].alignment = WD_ALIGN_PARAGRAPH.CENTER #가운데 정렬
            tp.vertical_alignment = WD_ALIGN_VERTICAL.CENTER #가운데 정렬
            # 열 3 4 5
            # 행 2 <14
...
#d = Document('C:\\Users\\84429\\Desktop\\RF\\empty.docx')
#introduction(d)
